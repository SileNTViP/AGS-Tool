...
type
  ...
  TGameDta = class(TObject)
    private
      ...
      function ReadStringNull(FS: TFileStream): AnsiString;
...
begin
  Result := 0;
  FS.ReadBuffer(Result, SizeOf(Byte));
end;
...
function TGameDta.ReadStringNull(FS: TFileStream): AnsiString;
var
  buf: AnsiChar;
begin
  Result := '';
...
begin
  ...
  if Version >= kGuiVersion_unkn_106 then
    strcpy(tmpobj.scriptName, fgetstring_limit(FS, MAX_GUIOBJ_SCRIPTNAME_LEN))
  else
    tmpobj.scriptName[0] := #0;
  if Version >= kGuiVersion_unkn_108 then
  begin
    i := FS.Position;
    numev := ReadInt32(FS);
...
begin
  ...
  begin
    ...
    begin
      ...
      tmpgui.name := ReadStringNull(FS);
      tmpgui.clickEventHandler := ReadStringNull(FS);
    end;
  ...
  begin
    GUIObjectRead(FS, gui_version);
    if gui_version < kGuiVersion_272c then
    begin
      SetLength(buf, 200);
      FS.ReadBuffer(PAnsiChar(buf)^, 200);
    end else begin
      buflen := ReadInt32(FS);
      SetLength(buf, buflen);
      FS.ReadBuffer(PAnsiChar(buf)^, buflen);
    end;
    AddExtrString(AnsiString(buf));
  ...
  begin
    GUIObjectRead(FS, gui_version);
    if gui_version >= kGuiVersion_unkn_109 then
    begin
  ...
  begin
    ...
    begin
      GUIObjectRead(FS, gui_version);
      if gui_version >= kGuiVersion_unkn_104 then
  ...
  // text boxes
  if gui_version >= kGuiVersion_222 then
  begin
...
//  FS.Seek(stlen, soFromCurrent);
  ...
  begin
    FS.ReadBuffer(GSS.GUID, MAX_GUID_LENGTH);
    FS.ReadBuffer(GSS.saveGameFileExtension, MAX_SG_EXT_LENGTH);
    FS.ReadBuffer(GSS.saveGameFolderName, MAX_SG_FOLDER_LEN);
  end;
  ...
  for i := 0 to GSS.Base.NumFonts - 1 do // Skip font filenames
  begin
    FS.ReadBuffer(stlen, SizeOf(Integer));
    SetLength(bufstr, stlen);
    FS.ReadBuffer(PAnsiChar(bufstr)^, stlen);
//    FS.Seek(stlen, soFromCurrent);
  end;
...
begin
  ...
  begin
    ...
    GSS.InvInfo[i].Flags := ReadInt32(FS);
  end;
end;
...
begin
  ...
  begin
    ...
    GSS.MCurs[i].Flags := ReadInt32(FS);
  end;
end;
